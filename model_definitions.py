...
def initialize_all_models(input_dimension: int, 
    ...
    unique_model_names = {
        ...
        'wide_relu_ann': 'Wide ReLU ANN',
        'deep_relu_ann': 'Deep ReLU ANN',
        'lookup_table_model': 'Lookup Table Model',
        'spline_ann': 'Spline ANN',
        'abel_spline': 'ABEL Spline'
    }

    models = [
        (create_linear_model(**common_args), unique_model_names['linear_model']),
        (create_wide_relu_ann(hidden_units=hidden_units_wide, **common_args), unique_model_names['wide_relu_ann']),
        (create_deep_relu_ann(hidden_units=hidden_units_deep, hidden_layers=hidden_layers, **common_args), unique_model_names['deep_relu_ann']),
        (LookupTableModel(partition_num=1, default_val=-1., **common_args), unique_model_names['lookup_table_model'])
    ]

    for partition_num in [1,2,4,8,10]:
        models.append((SplineANN(partition_num=partition_num, **common_args), f"Spline ANN (Partition {partition_num})"))
        models.append((LookupTableModel(partition_num=partition_num, default_val=-1., **common_args), f"Lookup Table Model (Partition {partition_num})"))
        models.append((ABELSpline(partition_num=partition_num, num_exps=num_exps, **common_args), f"ABEL Spline (Partition {partition_num})"))
